...
func Help(w io.Writer, template string, data interface{}) {
	...
	c.Printf("\n Commands:\n")
	fmt.Print("    <kwklink,..>                      - Open a kwklink or tag or execute the kwklink in the default app or script                                                               runtime.\n")
	...
	fmt.Print("    title      <kwklink> [text]       - Used to make a title for the resource\n")
	fmt.Println()
	...
	fmt.Print("    cat        <kwklink>              - Gets URIs without navigating. (Copies first to clipboard)\n")

	c.Printf("\n Community:\n")
	fmt.Print("    cd    	  [username|tag]         - *Navigate to another users or tag. Used when searching or listing \n")
	fmt.Print("    fork      <kwklink> <uri> [oldstr] [newstr] - makes a copy of a kwklink and optionally replaces a string. \n")
